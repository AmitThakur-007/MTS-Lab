...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const fetchPendingAttendance = async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) setPendingAttendanceCount(data.length);
    } catch {
      // ignore
    }
  };
  ...
  const handleNotificationClick = async (item: any) => {
    ...
    else if (item.repairId) navigate(`/dashboard/repairs/${item.repairId}`);
    else if (item.type === 'TRANSFER_REQUEST' && user?.role === 'TECHNICIAN') navigate('/dashboard');
  ...
  const navItems = [
    ...
    { name: 'Security & System Control', path: '/dashboard/super-admin', icon: ShieldAlert, roles: ['SUPER_ADMIN'] },
        ...
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between h-20 sm:h-24 px-6 sm:px-8 border-b border-slate-100/80 shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-black rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-black/20 shrink-0">M</div>
              <div className="min-w-0"><span className="text-lg sm:text-xl font-black tracking-tight text-slate-900 block truncate">MTS LAB</span><span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block -mt-1 truncate">Repair Systems</span></div>
            </div>
          ...
          <nav className="flex-1 px-3.5 sm:px-4 py-4 space-y-1 overflow-y-auto">
            ...
            {navItems.map((item) => (
              <NavLink key={item.path} to={item.path} end={item.path === '/dashboard'} className={({ isActive }) => cn('flex items-center justify-between px-3.5 py-3 text-xs sm:text-sm font-bold rounded-2xl transition-all group min-h-[44px]', isActive ? 'bg-slate-950 text-white shadow-lg shadow-black/15 font-extrabold' : 'text-slate-600 hover:bg-slate-100/90 hover:text-slate-950')} onClick={() => setIsSidebarOpen(false)}>
                <span className="flex items-center gap-2"><item.icon className="h-4 w-4 shrink-0" /><span>{item.name}</span></span>
                {item.name === 'Attendance' && pendingAttendanceCount > 0 && <span className="flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-black rounded-full bg-amber-500 text-white shadow-xs animate-pulse">{pendingAttendanceCount}</span>}
                {item.path.includes('new') && <Plus className="h-4 w-4 opacity-40 shrink-0" />}
              </NavLink>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-60 sm:w-64 rounded-2xl p-2 shadow-2xl border-slate-200" align="end">
                <DropdownMenuGroup><DropdownMenuLabel className="p-3"><div className="flex flex-col space-y-0.5"><p className="text-xs sm:text-sm font-extrabold text-slate-900 truncate">{user?.name}</p><p className="text-xs font-medium text-slate-400 truncate">{user?.email}</p></div></DropdownMenuLabel></DropdownMenuGroup>
                <DropdownMenuSeparator />
                <DropdownMenuGroup className="p-1 space-y-0.5"><DropdownMenuItem onClick={() => navigate('/dashboard/settings')} className="rounded-xl h-10 px-3 font-bold text-xs gap-2 cursor-pointer"><Settings className="h-3.5 w-3.5 text-slate-500" /> Account Settings</DropdownMenuItem><DropdownMenuItem onClick={() => navigate('/#contact')} className="rounded-xl h-10 px-3 font-bold text-xs gap-2 cursor-pointer"><HelpCircle className="h-3.5 w-3.5 text-slate-500" /> Help & Support</DropdownMenuItem></DropdownMenuGroup>
                <DropdownMenuSeparator /><DropdownMenuItem onClick={handleLogout} className="rounded-xl h-10 px-3 font-bold text-xs text-rose-600 focus:bg-rose-50 gap-2 cursor-pointer"><LogOut className="h-3.5 w-3.5" /> Log Out System</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </header>
