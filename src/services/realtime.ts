...
class RealtimeService {
  ...
  public connect() {
    ...
    if (this.supabaseConnected && this.supabaseChannel) {
      this.setStatus('connected');
      return;
    }
    this.initSupabaseRealtimeListeners();
...
export function useRealtimeSync(
  ...
  const isTabVisibleRef = useRef(typeof document !== 'undefined' ? document.visibilityState === 'visible' : true);
  ...
  // Tab visibility listener — defer refetches when tab is in background
  useEffect(() => {
    if (typeof document === 'undefined') return;
    const handleVisibilityChange = () => {
      const isVisible = document.visibilityState === 'visible';
      isTabVisibleRef.current = isVisible;
      if (isVisible && pendingEventRef.current && callbackRef.current) {
        const evt = pendingEventRef.current;
        pendingEventRef.current = null;
        if (debounceTimerRef.current) clearTimeout(debounceTimerRef.current);
        debounceTimerRef.current = setTimeout(() => {
          if (callbackRef.current) callbackRef.current(evt);
        }, 150);
      }
    };
  ...
  useEffect(() => {
    ...
    const unsubscribeEvents = realtimeService.subscribe(entities, (event) => {
      ...
      // When tab is hidden in background, stash event and do not fire expensive API fetches
