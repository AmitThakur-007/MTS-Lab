...
import { InventoryFilesManager } from '@/components/inventory/InventoryFilesManager';
...
export function getDeviceType(item: any): 'Smartphone' | 'Tablet' | 'iPad' | 'Laptop' | 'TV' | 'Other' {
  const combined = `${item.name || ''} ${item.model || ''} ${item.subcategory || ''} ${item.category || ''} ${item.compatibility || ''}`.toLowerCase();
  
  if (combined.includes('ipad')) return 'iPad';
  if (combined.includes('tablet') || combined.includes('tab ') || combined.includes('tab-')) return 'Tablet';
  if (combined.includes('laptop') || combined.includes('macbook') || combined.includes('notebook') || combined.includes('thinkpad')) return 'Laptop';
  if (combined.includes('tv') || combined.includes('television') || combined.includes('smart tv') || combined.includes('oled tv') || combined.includes('led tv')) return 'TV';
  if (
    combined.includes('phone') || 
    combined.includes('iphone') || 
    combined.includes('galaxy s') || 
    combined.includes('galaxy a') || 
    combined.includes('galaxy z') || 
    combined.includes('galaxy note') ||
    ...
    (item.category && ['Displays', 'Batteries', 'Charging Ports', 'Cameras', 'Back Panels', 'Flex Cables'].includes(item.category) && !combined.includes('laptop') && !combined.includes('tv'))
  ) {
    return 'Smartphone';
  }
...
export default function Inventory() {
  ...
  const [stats, setStats] = useState<{
    ...
    archivedValuation: 0,
    deviceCounts: { Smartphone: 0, Tablet: 0, iPad: 0, Laptop: 0, TV: 0, Other: 0 },
    recentTxCount: 0
  });
  ...
  const [deviceFilter, setDeviceFilter] = useState<'ALL' | 'Smartphone' | 'Tablet' | 'iPad' | 'Laptop' | 'TV' | 'Other'>('ALL');
  const [viewMode, setViewMode] = useState<'grid' | 'table'>('grid');
  const [sortBy, setSortBy] = useState<'updatedAt' | 'name' | 'stock' | 'price' | 'sku'>('updatedAt');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');
  const [folderTab, setFolderTab] = useState<'PARTS' | 'FILES'>('PARTS');
  ...
  const [isRestoreFolderOpen, setIsRestoreFolderOpen] = useState(false);
  ...
  const [folderToRestore, setFolderToRestore] = useState<{
    level: 'brand' | 'model' | 'category';
    name: string;
    brand: string;
    model?: string | null;
  ...
  // All active items (or archived if filter is selected, filtered by device type)
  const activeItems = useMemo(() => {
    ...
    if (deviceFilter !== 'ALL') {
      list = list.filter(i => getDeviceType(i) === deviceFilter);
    }
    return list;
  }, [items, stockFilter, deviceFilter]);
  ...
  const brandList = useMemo(() => {
    ...
    // Include custom folders matching the active / archive state
  ...
  }, [activeItems, customFolders, stockFilter]);
  ...
  }, [activeItems, customFolders, navPath.brand, stockFilter]);
  ...
  }, [activeItems, customFolders, navPath.brand, navPath.model, stockFilter]);
  ...
  const handleOpenRestoreFolder = (
    ...
    setIsRestoreFolderOpen(true);
  };

  // Restore Folder Submit (Cascades to all contained parts)
  const handleRestoreFolderSubmit = async () => {
    if (!folderToRestore || !folderToRestore.brand) return;

    setSubmitting(true);
    try {
      const res = await api.post('/inventory/restore-folder', {
        brand: folderToRestore.brand,
        model: folderToRestore.model || undefined,
        category: folderToRestore.category || undefined
      });

      toast.success(`✓ Restored folder "${folderToRestore.name}" and ${res.restoredCount || 0} contained parts to Active inventory.`);
      setIsRestoreFolderOpen(false);
      setFolderToRestore(null);
      fetchData();
    } catch (err: any) {
      toast.error(err.message || 'Failed to restore folder');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleBulkRestoreSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await api.post('/inventory/bulk-restore', { ids: selectedIds });
      toast.success(`✓ Restored ${res.count || selectedIds.length} parts to Active inventory.`);
      setSelectedIds([]);
      fetchData();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 pb-32">
        ...
        <Card className="col-span-2 sm:col-span-1 rounded-3xl border-slate-200/80 bg-white shadow-xs">
          <CardContent className="p-4 md:p-5 flex flex-col justify-between h-full">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-black tracking-wider uppercase text-indigo-600">Stock Valuation</span>
            ...
            <div className="mt-2 flex flex-col">
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-500 font-medium mt-1">
                ...
                {stats.totalSellingValuation && stats.totalSellingValuation > 0 && (
                  <span className="text-indigo-600 font-bold">
                    Retail: {formatNPR(stats.totalSellingValuation)}
                  </span>
                )}
              </div>
              {stats.archivedProductsCount !== undefined && stats.archivedProductsCount > 0 && (
                <div className="mt-1 pt-1 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-400">
                  <span>Archived ({stats.archivedProductsCount})</span>
                  <span>{formatNPR(stats.archivedValuation || 0)}</span>
                </div>
              )}
            </div>
          </CardContent>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-1.5 p-1 bg-slate-100/90 rounded-2xl border border-slate-200/80 w-fit">
          ...
          >
            ...
            <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-600">
              {stats.totalProducts}
            </span>
          </button>
          <button
          ...
          >
            ...
            <span>Technical Files & Schematics</span>
          </button>
        </div>

        <div className="text-xs text-slate-500 font-medium flex items-center gap-1.5">
          <Info className="h-3.5 w-3.5 text-indigo-500 shrink-0" />
          <span>Folder Context: <strong className="text-slate-800 font-bold">{navPath.category ? `${navPath.brand} > ${navPath.model} > ${navPath.category}` : navPath.model ? `${navPath.brand} > ${navPath.model}` : navPath.brand || 'All Brands & Devices'}</strong></span>
        </div>
      </div>
          ...
          {folderTab === 'PARTS' && (
            <div className="space-y-3">
              <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
                  {[
                    ...
                    { id: 'ARCHIVED', label: `Archived (${stats.archivedProductsCount || 0})` }
                  ].map(tab => (
                    <button
                      key={tab.id}
              ...
              <div className="pt-2 border-t border-slate-100 flex flex-wrap items-center gap-1.5">
                ...
                {[
                  { id: 'ALL', label: 'All Hardware', icon: Boxes, count: stats.totalProducts },
                  { id: 'Smartphone', label: 'Smartphones', icon: Smartphone, count: stats.deviceCounts?.Smartphone || 0 },
                  { id: 'Tablet', label: 'Tablets', icon: Tablet, count: stats.deviceCounts?.Tablet || 0 },
                  { id: 'iPad', label: 'iPads', icon: Tablet, count: stats.deviceCounts?.iPad || 0 },
                  { id: 'Laptop', label: 'Laptops', icon: Laptop, count: stats.deviceCounts?.Laptop || 0 },
                  { id: 'TV', label: 'TVs & Displays', icon: Tv, count: stats.deviceCounts?.TV || 0 },
                  { id: 'Other', label: 'Other / Misc', icon: Package, count: stats.deviceCounts?.Other || 0 }
                ].map(dev => {
                  const Icon = dev.icon;
                  const isSelected = deviceFilter === dev.id;
                  return (
                    <button
                      key={dev.id}
                      onClick={() => setDeviceFilter(dev.id as any)}
                      className={cn(
                    ...
                    >
                      ...
                      {dev.count !== undefined && (
                        <span className={cn(
                          "ml-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-extrabold",
      ...
      {/* ========================================================= */}
      {/* TECHNICAL DOCUMENTATION & FILES VIEW */}
      {/* ========================================================= */}
      {folderTab === 'FILES' ? (
        <Card className="rounded-3xl border-slate-200/80 bg-white shadow-xs p-5 md:p-6">
          <div className="mb-4 pb-3 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Service manuals, pinouts, boardviews, and component datasheets for{' '}
                <span className="font-bold text-slate-800">
                  {navPath.category ? `${navPath.brand} > ${navPath.model} > ${navPath.category}` :
                   navPath.model ? `${navPath.brand} > ${navPath.model}` :
                   navPath.brand ? navPath.brand : 'All Brands & Hardware'}
                </span>
              </p>
            </div>
            {navPath.brand && (
              <Badge className="bg-indigo-50 text-indigo-700 border-indigo-100 font-bold text-xs px-2.5 py-1 w-fit">
                Filtered by {navPath.brand} {navPath.model ? `› ${navPath.model}` : ''}
              </Badge>
            )}
          </div>
                        ...
                        <div className="px-5 py-2.5 bg-slate-50/60 border-t border-slate-100 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                          {stockFilter === 'ARCHIVED' ? (
                            <Button
                              size="xs"
                              variant="outline"
                              onClick={() => handleOpenRestoreFolder('brand', b.brand, b.brand, null, null, b.itemCount)}
                              className="rounded-xl border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold text-xs flex items-center gap-1.5"
                            >
                        ...
                        <div className="px-5 py-2.5 bg-slate-50/60 border-t border-slate-100 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                          {stockFilter === 'ARCHIVED' ? (
                            <Button
                              size="xs"
                              variant="outline"
                              onClick={() => handleOpenRestoreFolder('model', m.model, navPath.brand || '', m.model, null, m.itemCount)}
                              className="rounded-xl border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold text-xs flex items-center gap-1.5"
                            >
                              <RotateCcw className="h-3.5 w-3.5" />
                              Restore Folder
                        ...
                        <div className="px-5 py-2.5 bg-slate-50/60 border-t border-slate-100 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                          {stockFilter === 'ARCHIVED' ? (
                            <Button
                              size="xs"
                              variant="outline"
                              onClick={() => handleOpenRestoreFolder('category', c.category, navPath.brand || '', navPath.model, c.category, c.itemCount)}
                              className="rounded-xl border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold text-xs flex items-center gap-1.5"
                            >
                              <RotateCcw className="h-3.5 w-3.5" />
                              Restore Folder
                            ...
                            <TableRow key={item.id} className={cn("border-slate-100 transition-colors", isSelected ? "bg-indigo-50/30" : "hover:bg-slate-50/50")}>
                              ...
                              <TableCell className="py-4 text-center">
                                <div className="flex items-center justify-center gap-1.5">
                                  {item.status === 'ARCHIVED' || stockFilter === 'ARCHIVED' ? (
                                    <Button
                                      size="xs"
                                      onClick={() => handleRestoreItem(item)}
                                      className="rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 font-bold flex items-center gap-1"
                                      title="Restore Part to Active Stock"
                                    >
                                      <RotateCcw className="h-3.5 w-3.5 mr-1" /> Restore Part
                                    </Button>
                                  ) : (
                                    <>
                                      <Button
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-900/95 backdrop-blur-md text-white px-5 py-3 rounded-2xl shadow-2xl border border-slate-700/80 flex items-center gap-4 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="flex items-center gap-2">
            {stockFilter === 'ARCHIVED' && canManage ? (
              <Button
                size="xs"
              ...
              >
                <RotateCcw className="h-3.5 w-3.5 mr-1" /> Restore Selected ({selectedIds.length})
              </Button>
            ) : (
              <>
                <Button
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent className="max-w-2xl rounded-3xl p-6 max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-slate-900 flex items-center gap-2">
              <Package className="h-5 w-5 text-indigo-600" /> Part Specifications & Documentation
            </DialogTitle>
          </DialogHeader>
          ...
          {selectedItem && (
            <div className="space-y-4 py-2">
              ...
              {/* Technical Attachments & Schematics */}
              <div className="pt-3 border-t border-slate-100 space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-black text-slate-900 flex items-center gap-1.5 uppercase tracking-wider">
                    <FileText className="h-4 w-4 text-indigo-600" /> Attached Schematics, Datasheets & Pinouts
                  </h4>
                </div>
                <InventoryFilesManager
      ...
      {/* ========================================================= */}
      {/* MODAL 12: RESTORE FOLDER CONFIRMATION */}
      {/* ========================================================= */}
      <Dialog open={isRestoreFolderOpen} onOpenChange={setIsRestoreFolderOpen}>
        <DialogContent className="max-w-md rounded-3xl p-6">
          <DialogHeader>
            <DialogTitle className="text-xl font-black text-emerald-700 flex items-center gap-2">
              <RotateCcw className="h-5 w-5" /> Restore {folderToRestore?.level} Folder?
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-500">
              Restore <span className="font-bold text-slate-900">{folderToRestore?.name}</span> and all contained parts back to active stock?
            </DialogDescription>
          </DialogHeader>

          <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 space-y-2 text-xs text-emerald-800">
            <p className="font-bold">✓ Restoration Plan:</p>
            <p>
              This folder and {folderToRestore?.count !== undefined ? `${folderToRestore.count} parts` : 'all archived parts within it'} will be restored to <strong>ACTIVE</strong> status and immediately available for workshop repairs.
            </p>
          </div>

          <DialogFooter className="pt-3 gap-2">
            <Button type="button" variant="outline" onClick={() => setIsRestoreFolderOpen(false)} className="h-11 rounded-2xl border-slate-200">
              Cancel
            </Button>
            <Button 
              type="button" 
              disabled={submitting} 
              onClick={handleRestoreFolderSubmit} 
              className="h-11 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-6"
            >
              {submitting ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <RotateCcw className="h-4 w-4 mr-2" />}
              Restore Folder & Parts
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
