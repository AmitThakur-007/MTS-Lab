...
export default function About() {
  ...
  const canEditPhotos = !!user && (user.role === 'SUPER_ADMIN' || user.role === 'ADMIN');

  const [photoOverrides, setPhotoOverrides] = useState<Record<string, string>>({});
  const [imgErrors, setImgErrors] = useState<Record<string, boolean>>({});
  const [isUploading, setIsUploading] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [activeTarget, setActiveTarget] = useState<string>('ceo');

  const getTargetKey = (shortRole: string) => {
    if (shortRole === 'CEO') return 'ceo';
    if (shortRole === 'Founder') return 'founder';
    return 'technical-head';
  };

  const getChangeLabel = (shortRole: string) => {
    if (shortRole === 'CEO') return 'Change CEO Photo';
    if (shortRole === 'Founder') return 'Change Founder Photo';
    return 'Change Technical Head Photo';
  };

  const getUploadLabel = (shortRole: string) => {
    if (shortRole === 'CEO') return 'Upload CEO Photo';
    if (shortRole === 'Founder') return 'Upload Founder Photo';
    return 'Upload Technical Head Photo';
  };

  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!canEditPhotos) {
      toast.error('Unauthorized. Only administrators can update leadership photos.');
      return;
    }
    ...
    // Validate client-side image MIME type
    const validTypes = ['image/jpeg', 'image/png', 'image/webp'];
    if (!validTypes.includes(file.type)) {
      toast.error('Invalid image format. Only JPEG, PNG, and WebP images are allowed.');
      return;
    }
    ...
    setIsUploading(activeTarget);
    try {
      const headers: Record<string, string> = {};
      if (token) headers['Authorization'] = `Bearer ${token}`;

      const res = await fetch('/api/admin/team/update-photo', {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      if (!res.ok) {
        throw new Error(data.message || data.error || 'Failed to update leadership photo.');
      }
      ...
      const roleLabel = activeTarget === 'ceo'
        ? 'CEO'
        : activeTarget === 'founder'
        ? 'Founder'
        : 'Technical Head';
      toast.success(`${roleLabel} photo updated successfully! Old photo has been replaced.`);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 font-sans text-slate-800 leading-relaxed selection:bg-indigo-600 selection:text-white flex flex-col antialiased">
            ...
            >
              <div className="p-5 sm:p-6 space-y-5">
                ...
                <div className="relative aspect-[4/5] w-full rounded-2xl overflow-hidden bg-slate-100 shadow-inner">
                  {imgErrors[getTargetKey(leader.shortRole)] ? (
                    <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-indigo-950 via-slate-900 to-purple-950 text-white p-5 text-center">
                      <div className="w-14 h-14 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center text-lg font-black text-indigo-300 border border-white/15 mb-2 shadow-inner">
                        {leader.name.split(' ').map(n => n[0]).join('')}
                      </div>
                      <p className="font-extrabold text-xs sm:text-sm text-white">{leader.name}</p>
                      <p className="text-[11px] text-indigo-300 font-semibold mb-3">{leader.position}</p>
                      {canEditPhotos && (
                        <button
                          type="button"
                          id={`upload-photo-${getTargetKey(leader.shortRole)}`}
                          onClick={() => {
                            const targetKey = getTargetKey(leader.shortRole);
                            setActiveTarget(targetKey);
                            fileInputRef.current?.click();
                          }}
                          className="px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[11px] shadow-lg transition-all flex items-center gap-1.5 cursor-pointer"
                        >
                          <Camera className="w-3.5 h-3.5" />
                          <span>{getUploadLabel(leader.shortRole)}</span>
                        </button>
                      )}
                    </div>
                  ) : (
                    <img
                      src={photoOverrides[getTargetKey(leader.shortRole)] || leader.image}
                      alt={leader.alt}
                      loading="lazy"
                      decoding="async"
                      ...
                      onError={() => {
                        const targetKey = getTargetKey(leader.shortRole);
                        setImgErrors(prev => ({ ...prev, [targetKey]: true }));
                      }}
                  ...
                  {/* Quick Change Photo Button - Strictly reserved for authorized administrators */}
                  {canEditPhotos && (
                    <button
                      type="button"
                      id={`change-photo-${getTargetKey(leader.shortRole)}`}
                      onClick={() => {
                        const targetKey = getTargetKey(leader.shortRole);
                        setActiveTarget(targetKey);
                        fileInputRef.current?.click();
                      ...
                      className="absolute bottom-3 right-3 px-2.5 py-1.5 rounded-xl bg-white/95 hover:bg-white text-slate-800 hover:text-indigo-600 shadow-md border border-slate-200/90 transition-all z-20 flex items-center gap-1.5 text-[11px] font-bold tracking-tight backdrop-blur-md cursor-pointer"
                      title={getChangeLabel(leader.shortRole)}
                    >
                      {isUploading === getTargetKey(leader.shortRole) ? (
                        <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-600" />
                      ) : (
                        <>
                          <Camera className="w-3.5 h-3.5 text-indigo-600" />
                          <span>{getChangeLabel(leader.shortRole)}</span>
                        </>
                      )}
                    </button>
                  )}
      ...
      {/* Hidden file input for leadership photo upload (Authorized Administrative Staff only) */}
      {canEditPhotos && (
        <input
          ref={fileInputRef}
          type="file"
