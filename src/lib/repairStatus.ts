...
export interface RepairDashboardMetrics {
  total: number; // Active repairs (excluding cancelled)
  activeTotal: number; // Active repairs (excluding cancelled)
  totalRecords: number; // Historical all repair records
  pending: number;
  received: number;
  inProgress: number;
 ...
 * Excludes cancelled repairs from the active total.
 */
export function calculateRepairMetrics(repairs: any[]): RepairDashboardMetrics {
  ...
  const activeTotal = Math.max(0, totalRecords - cancelled);
