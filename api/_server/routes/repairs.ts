...
router.get('/', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // SERVER-SIDE TECHNICIAN ISOLATION (Definitive Fix for IDOR and Workbench Visibility)
    // 1. Regular TECHNICIANS must NEVER be able to see or query other technicians' repairs.
    // 2. When on the technician bench (scope=active) or assignedToMeOnly=true, the logged-in technician's own assigned repairs are strictly returned.
    // 3. If a manager or admin explicitly queries a specific technicianId (or UNASSIGNED), honor that parameter; otherwise for technician bench views, enforce isolation.
    if (role === 'TECHNICIAN' || isAssignedToMeOnly || (isTechRole && scopeStr === 'active' && !technicianId)) {
      query = query.eq('technicianId', req.user!.id);

      // Technicians must NEVER query or see cancelled devices across any views or date presets
    ...
    // Date filtering: support presets and custom date ranges with precise Nepal timezone bounds
...
const handleRepairUpdate = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Prevent unauthorized updates to already cancelled repairs
...
router.patch('/:id/technician-update', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (role === 'TECHNICIAN') {
      ...
      // 2. Status restrictions: Delivered, Ready for Pickup, Re-Problem, Cancelled cannot be set by technicians
      const FORBIDDEN_TECHNICIAN_STATUSES = [
        'DELIVERED',
        'READY_FOR_PICKUP',
        'READY',
...
// ----------------------------------------------------
// 13b. POST /:id/cancel & PATCH /:id/cancel — Cancel Repair Device/Service
// Accessible to SUPER_ADMIN, ADMIN, MANAGER, RECEPTIONIST
// ----------------------------------------------------
const handleCancelRepair = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const fullCancellationSummary = extraNote 
      ? `${cancellationReason}: ${extraNote}`
      : cancellationReason;
    ...
    // Safely append to remarks without destroying existing remarks or problem description
    ...
    // Notify assigned technician if any that device was cancelled
    if (existingRepair.technicianId) {
      try {
        await createNotification({
          userId: existingRepair.technicianId,
          title: `🚫 Job #${existingRepair.repairNumber} Cancelled`,
...
router.post('/:id/cancel', authenticate, authorize(['SUPER_ADMIN', 'ADMIN', 'MANAGER', 'RECEPTIONIST']), handleCancelRepair);
router.patch('/:id/cancel', authenticate, authorize(['SUPER_ADMIN', 'ADMIN', 'MANAGER', 'RECEPTIONIST']), handleCancelRepair);

// Re-open / Restore Cancelled Repair (SUPER_ADMIN, ADMIN, MANAGER)
router.post('/:id/reopen', authenticate, authorize(['SUPER_ADMIN', 'ADMIN', 'MANAGER']), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/:id/assign', authenticate, authorize(['SUPER_ADMIN', 'ADMIN', 'MANAGER', 'LEAD_TECHNICIAN', 'RECEPTIONIST']), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const normalizedTechId = (technicianId && String(technicianId).trim() !== '' && String(technicianId).toLowerCase() !== 'unassigned' && String(technicianId).toLowerCase() !== 'null')
    ...
    const { data: updated, error } = await supabaseAdmin
      ...
      .update({
        ...
        assignedAt: normalizedTechId ? new Date().toISOString() : null,
        assignedById: normalizedTechId ? req.user!.id : null,
        assignedByName: normalizedTechId ? (req.user!.name || 'Staff') : null,
        updatedAt: new Date().toISOString(),
      })
      .eq('id', id)
      .select('*, customer:Customer(*), technician:User!Repair_technicianId_fkey(id, name, role, email)')
      .single();
      ...
      await supabaseAdmin.from('RepairLog').insert([
        {
          ...
          message: normalizedTechId
            ? `Assigned to technician: ${techName || 'Technician'} by ${req.user!.name || 'Staff'}`
            : `Repair unassigned by ${req.user!.name || 'Staff'}`,
          createdAt: new Date().toISOString(),
        },
      ]);
      await broadcastServerChange('RepairLog', 'CREATE', logId);
