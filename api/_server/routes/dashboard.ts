...
router.get('/overview', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 1. Fetch repairs from Supabase for overview calculations (limit to recent 1000 records to optimize memory)
    ...
    // 5. Fetch Inventory Items for stock alerts (limit to 200)
    ...
    // 6. Fetch Battery Warranties active and total counts using lightweight exact head counts
    const { count: totalWarrantiesCount } = await supabaseAdmin
      .from('BatteryWarranty')
      .select('*', { count: 'exact', head: true });

    const { count: activeWarrantiesCount } = await supabaseAdmin
      .from('BatteryWarranty')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'ACTIVE');
    ...
    const myRepairs = allRepairs.filter((r) =>
      ...
      !['CANCELLED', 'CANNOT_REPAIR'].includes((r.status || '').toUpperCase())
    );
    const myActiveRepairs = myRepairs.filter((r) =>
      !['COMPLETED', 'DELIVERED'].includes((r.status || '').toUpperCase())
    );
