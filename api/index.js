...
router.post("/2fa/verify", async (req, res) => {
  try {
    ...
    const { data: otps, error: otpErr } = await supabaseAdmin.from("OTPVerification").select("*").eq("id", mfaTicket).eq("purpose", "LOGIN_2FA").eq("isUsed", false).limit(1);
...
router.post("/2fa/resend", async (req, res) => {
  try {
    ...
    const { data: otps } = await supabaseAdmin.from("OTPVerification").select("*").eq("id", mfaTicket).limit(1);
...
router6.get("/stats", authenticate, async (req, res) => {
  try {
    const { data: records } = await supabaseAdmin.from("Repair").select("isCourierIn, isCourierOut, courierStatus, isReturnCourierDispatched").or("isCourierIn.eq.true,isCourierOut.eq.true");
    let totalIncoming = 0;
    let totalOutgoing = 0;
    let inTransit = 0;
    let delivered = 0;
...
router6.get("/eligible-repairs", authenticate, async (req, res) => {
  try {
    const { data: repairs } = await supabaseAdmin.from("Repair").select("id, repairNumber, customerName, customerPhone, customerAddress, deviceBrand, deviceModel, status, totalPaid, estimatedCost").in("status", ["COMPLETED", "READY_FOR_DELIVERY", "REPAIRED", "READY"]).eq("isCourierOut", false).order("createdAt", { ascending: false }).limit(30);
    return res.json(repairs || []);
  } catch (err) {
...
router6.get("/filters-metadata", authenticate, async (req, res) => {
  try {
    const { data: repairs } = await supabaseAdmin.from("Repair").select("courierCompany, returnCourierCompany, originDistrict, destinationDistrict").or("isCourierIn.eq.true,isCourierOut.eq.true");
...
router6.get("/search-customers", authenticate, async (req, res) => {
  try {
    ...
    const { data: customers } = await supabaseAdmin.from("Customer").select("name, phone, address, district, municipality").or(`phone.ilike.%${term}%,name.ilike.%${term}%`).limit(10);
    return res.json(customers || []);
  } catch (err) {
    return res.json([]);
  }
...
router6.post("/check-duplicate-awb", authenticate, async (req, res) => {
  try {
    ...
    const { data: existing } = await supabaseAdmin.from("Repair").select("id, repairNumber").or(`courierTrackingNumber.eq.${awb},returnCourierTrackingNumber.eq.${awb}`).limit(1);
    return res.json({
...
router6.post("/incoming", authenticate, async (req, res) => {
  try {
    ...
    const repairNumber = `MTS-${(/* @__PURE__ */ new Date()).getFullYear()}-${Date.now().toString().slice(-4)}`;
    const newRepair = {
      id: repairId,
      repairNumber,
      ...
      courierCompany,
      courierTrackingNumber,
      courierDate: (/* @__PURE__ */ new Date()).toISOString(),
      senderName,
      senderPhone,
      originDistrict,
      originAddress,
...
router6.post("/outgoing", authenticate, async (req, res) => {
  try {
    ...
    const { data: updated, error } = await supabaseAdmin.from("Repair").update({
      ...
      returnCourierTrackingNumber,
      returnCourierNotes: notes,
      returnCourierDispatchDate: (/* @__PURE__ */ new Date()).toISOString(),
      isReturnCourierDispatched: true,
      returnCourierDispatchedAt: (/* @__PURE__ */ new Date()).toISOString(),
      returnCourierDispatchedById: req.user.id,
      returnCourierDispatchedByName: req.user.name,
...
router6.post("/bulk-status", authenticate, async (req, res) => {
  try {
    ...
    const { error } = await supabaseAdmin.from("Repair").update({ courierStatus: status, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }).in("id", ids);
...
router6.post("/bulk-archive", authenticate, async (req, res) => {
  try {
    ...
    const { error } = await supabaseAdmin.from("Repair").update({ courierStatus: "ARCHIVED", updatedAt: (/* @__PURE__ */ new Date()).toISOString() }).in("id", ids);
...
router6.delete("/:id", authenticate, async (req, res) => {
  try {
    const { id } = req.params;
    const { error } = await supabaseAdmin.from("Repair").update({ isCourierIn: false, isCourierOut: false, courierStatus: "DELETED" }).eq("id", id);
