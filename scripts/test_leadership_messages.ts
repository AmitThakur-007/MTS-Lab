...
async function runLeadershipMessagesTests() {
  ...
  assert(aboutContent.includes("const sabitaPhoto = '/images/team/sabita-thakur.jpg';") || aboutContent.includes("sabita-thakur.jpg"), "References sabita-thakur.jpg");
  assert(aboutContent.includes("const manishPhoto = '/images/team/manish-sharma.jpg';") || aboutContent.includes("manish-sharma.jpg"), "References manish-sharma.jpg");
  assert(aboutContent.includes("const amitPhoto = '/images/team/amit-sharma.jpg';") || aboutContent.includes("amit-sharma.jpg"), "References amit-sharma.jpg");

  // Verify that each leader object references their own photo
  const sabitaIdx = aboutContent.indexOf("name: 'Sabita Thakur'");
  const manishIdx = aboutContent.indexOf("name: 'Manish Thakur'") !== -1 ? aboutContent.indexOf("name: 'Manish Thakur'") : aboutContent.indexOf("name: 'Manish Sharma'");
  const amitIdx = aboutContent.indexOf("name: 'Amit Thakur'") !== -1 ? aboutContent.indexOf("name: 'Amit Thakur'") : aboutContent.indexOf("name: 'Amit Sharma'");

  assert(sabitaIdx !== -1, "Found Sabita Thakur entry");
  assert(manishIdx !== -1, "Found Founder entry");
  assert(amitIdx !== -1, "Found Technical Head entry");
